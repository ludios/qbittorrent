...
namespace
{
    ...
    std::optional<SplitToFriendlyUnitResult> splitToFriendlyUnit(const qint64 bytes, const int unitThreshold = 1000)
    {
        if (bytes < 0)
            return std::nullopt;
        ...
        while ((value >= unitThreshold) && (i < static_cast<int>(Utils::Misc::SizeUnit::ExaByte)))
        {
            value /= 1000;
            ++i;
        }
